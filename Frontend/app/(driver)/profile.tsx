...
import { clearTokens, saveThemePreference, getThemePreference } from "@/services/storage";
...
export default function DriverProfile() {
    ...
    const getVerificationBadge = () => {
        ...
        let bgClass = "bg-primary/10 border border-primary/20";
        ...
        return (
            <View className={`${bgClass} px-4 py-1.5 rounded-full mt-3 flex-row items-center`}>
                <Feather name={iconName} size={12} color={status === "verified" ? theme.colors.success : status === "rejected" ? theme.colors.danger : status === "under_review" ? "#3B82F6" : status === "pending" ? "#F59E0B" : theme.colors.primary} className="mr-1.5" />
            ...
            <SafeAreaView className="flex-1 px-5 pt-3">
                ...
                >
                        ...
                        {vehicle ? (
                            <View className="gap-y-3">
                                <View className="flex-row items-center justify-between border-b border-white/[0.04] pb-2 px-1">
                                    ...
                                    <Text className="text-foreground text-sm font-semibold">{vehicle.make} {vehicle.model}</Text>
                                </View>
                                <View className="flex-row items-center justify-between border-b border-white/[0.04] pb-2 px-1">
                                    <Text className="text-muted text-xs">Color / Type</Text>
                                    <Text className="text-foreground text-sm font-semibold capitalize">{vehicle.color} ({vehicle.vehicleType})</Text>
                                </View>
                                <View className="flex-row items-center justify-between border-b border-white/[0.04] pb-2 px-1">
                                    <Text className="text-muted text-xs">License Plate</Text>
                                    <Text className="text-foreground text-sm font-semibold">{vehicle.plate}</Text>
                                </View>
                                <View className="flex-row items-center justify-between px-1">
                                    <Text className="text-muted text-xs">Passenger Capacity</Text>
                                    <Text className="text-foreground text-sm font-semibold">{vehicle.capacity} Seats</Text>
                                </View>
                            </View>
                        ) : (
                            <Text className="text-muted text-sm text-center py-2">No vehicle linked to profile.</Text>
                        )}
                    </View>
                    ...
                    <View className={`${glassCard} p-5 shadow-xl mb-6 gap-y-3`}>
                        <View className="flex-row items-center justify-between border-b border-white/[0.04] pb-2 px-1">
                            <Text className="text-muted text-xs">Driver License</Text>
                            <Text className="text-foreground text-sm font-semibold">{formatDateStr(driver?.license?.expiryDate)}</Text>
                        </View>
                        <View className="flex-row items-center justify-between border-b border-white/[0.04] pb-2 px-1">
                            <Text className="text-muted text-xs">Insurance Policy</Text>
                            <Text className="text-foreground text-sm font-semibold">{formatDateStr(vehicle?.insurance?.expiryDate)}</Text>
                        </View>
                        <View className="flex-row items-center justify-between border-b border-white/[0.04] pb-2 px-1">
                            <Text className="text-muted text-xs">PUC Expiration</Text>
                            <Text className="text-foreground text-sm font-semibold">{formatDateStr(vehicle?.puc?.expiryDate)}</Text>
                        </View>
                        <View className="flex-row items-center justify-between px-1">
                            <Text className="text-muted text-xs">Commercial Permit</Text>
                            <Text className="text-foreground text-sm font-semibold">{formatDateStr(vehicle?.permit?.expiryDate)}</Text>
                        </View>
                    </View>
